...
<?php
            ...
            if(isset($_POST['search'])&&!empty($_POST['etud'])){
				...
				$req="select s.ID_User,s.Login,p.Nom,p.Photo,p.Prenom,p.Email from user s,etudiant p where p.Nom like '%$search%' or p.Prenom like '%$search%' and s.ID_User=p.FK_ID_USER GROUP by p.Prenom";
			}else{
				$req="select s.ID_User,s.Login,p.Nom,p.Photo,p.Prenom,p.Email from user s,etudiant p where s.ID_User=p.FK_ID_USER";
			}
